...
const handleProcess = async () => {
  ...
  if (bgColor) {
    ...
    eelse if (activeTool === "enhance" || activeTool === "upscale") {
  const img = document.createElement("img");
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e8e8f0", fontFamily: "system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", gap: 0, padding: "28px 32px", maxWidth: 1100, margin: "0 auto", width: "100%" }}>
        ...
        <div style={{ flex: 1, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px 0 0 16px", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div onDrop={handleDrop} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onClick={() => !uploadedImage && fileRef.current?.click()} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", cursor: uploadedImage ? "default" : "pointer", background: dragging ? "rgba(124,58,237,0.08)" : "transparent", minHeight: 380, position: "relative" }}>
          ...
          {uploadedImage && (
            ...
            <button
              onClick={() => {
                ...
                link.download = "pixora-removed.png";
                link.click();
              }}
              style={{
                flex: 1,
        ...
        <div style={{ width: 290, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", borderLeft: "none", borderRadius: "0 16px 16px 0", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: 20, flex: 1 }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.3)", letterSpacing: 1, marginBottom: 12 }}>
  BACKGROUND COLOR
</p>
    ...
    {["transparent", "#ffffff", "#000000", "#ef4444", "#3b82f6", "#22c55e", "#f59e0b", "#8b5cf6"].map((color) => (
      <button
        ...
        style={{
          ...
          border: (bgColor === color || (color === "transparent" && !bgColor))
            ? "2px solid #a78bfa"
    ...
    {bgColor && (
      ...
      >
        ✕ Reset
            ...
            <button onClick={handleProcess} disabled={!uploadedImage || processing} style={{ width: "100%", padding: 14, background: uploadedImage && !processing ? "linear-gradient(135deg, #7c3aed, #2563eb)" : "rgba(255,255,255,0.06)", border: "none", borderRadius: 12, color: uploadedImage && !processing ? "#fff" : "rgba(255,255,255,0.3)", fontSize: 14, fontWeight: 700, cursor: uploadedImage && !processing ? "pointer" : "not-allowed" }}>
